...
def openai_bag(index, content, finish_reason=""):
    '''
        openai的接口样式
    '''
    # 生成唯一的请求 ID
    ...
    return {"id":request_id,
            ...
            "choices":[
                {"index":index,"delta":{"content":content},"logprobs":"","finish_reason":finish_reason}
            ]
        }
...
def prefill_and_generate(model, tokenizer, inputs, max_new_tokens=10000, use_cuda_graph: bool = True,
                         ...
                         stream_flag = False):
    '''
        修改了原有的prefill_and_generate方法，新增了带有流式响应返回方式,
        新增stream_flag流式标记, 
        stream_flag=True表示开启流式生成响应
        最终返回一个迭代器
    '''
    import os
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    ...
    with torch.no_grad():
        if not stream_flag:
            ...
            if force_think:
                print("<think>\n")
                yield openai_bag(bag_index, "<think>")    #   返回思考标记<think>
                bag_index += 1

            print(stream.put(next_token.item()), end="", flush=True)
            yield openai_bag(bag_index, tokenizer.decode(next_token))    #   返回起始字段内容
            ...
            for i in range(1, max_new_tokens):
                ...
                if next_token[0].item() == tokenizer.eos_token_id or tokenizer.decode(next_token) == '<|im_end|>':
                    print(stream.end(), end="", flush=True)
                    bag_index += 1
                    yield openai_bag(bag_index, "", finish_reason="stop")   #   返回结束token内容
                    break
                else:
                    print(stream.put(next_token.item()), end="", flush=True)
                    bag_index += 1
                    yield openai_bag(bag_index, tokenizer.decode(next_token))   #   返回生成的token内容
                cache_position += 1
                position_ids = cache_position.unsqueeze(0)
